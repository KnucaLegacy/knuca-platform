...
public class RoomDateBrackets {
    ...
    private Set<Circumstance> parseBrackets() {
        ...
        Bracket first = splitToBrackets();
        for (Bracket current = first; current != null; current = current.next) {
            current.parse();
        }
        ...
        for (Bracket current = first; current != null; current = current.next) {
            if (current.dates.isEmpty() && current.next != null) {
                circumstance = new Circumstance();
                if (current.prev != null && !localDates.isEmpty()){
                    if (!current.next.dates.isEmpty()) {
                        rooms = new HashSet<>();
                    }
                }
                ...
                if (!current.dates.isEmpty() && !circumstance.getDates().isEmpty()){
                    rooms = new HashSet<>();
                }
                if (rooms.isEmpty()) {
                    circumstance = new Circumstance();
                }
                localDates = new HashSet<>();
                rooms.add(current.room);
                localDates.addAll(current.dates);
